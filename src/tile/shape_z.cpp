...
ShapeZ::ShapeZ(const tile_sprites& type)
{
    resetShape(type);
    INFOLOG("ShapeZ construct success");
}

ShapeZ::~ShapeZ()
{ 
    INFOLOG("ShapeZ ~ShapeZ success");
}

void ShapeZ::resetShape(const tile_sprites& type, const int& defaultTileX, const int& defaultTileY)
{ 
    ...
    for (size_t i = 0; i < 2; i++)
    {
        ...
        if(defaultTileX == -1)
        {
            temp->setRelativeCoordinate((g_tile_board_middle+i)*g_tile_length, g_tile_length);
        }
        else
        {
            temp->setAbsoluteCoordinate(defaultTileX+i*g_tile_length, defaultTileY + g_tile_length);
        }
        m_tile_vector.push_back(temp);
    }
}

std::vector<Tile*> ShapeZ::getNextTilesInfo()
{
...
void ShapeZ::updateCurrentShape()
{
    // I只有两种形态
    switch (m_current_shape)
    {
